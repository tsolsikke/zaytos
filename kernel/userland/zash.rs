...
//!
//! - **「回して待つ」**——**いまは待つ。** **1 セッションのシステムコールは
//!   1,377,679 回から 2,969 回になった**（実測。584 バイトに対して 5.08 倍）
//! - **「カーネル側で眠らせる形はユーザープロセスのスケジューラを要求する」**
//!   ——**要求しなかった。** **カーネルのタスクスケジューラで足りた**
//!   （`TaskState::Waiting` と、BSP 用のアイドルタスク）
//! - **「カーネルが待つ形も採れない——`read` の中で眠ると BKL を保持したまま
//!   眠ることになる」**——**採れた。** **理由づけ自体が誤っていた**
//!   ——**`ADR-0036` が「解いてから眠る」踊りを既に持っており**（`SYS_SPAWN` と
//!   virtio が使っている）、**`sys_read` はガードを引数で受け取っているので
//!   落とすだけでよかった。** **`ADR-0023` の Addendum §2 が禁じているのは
//!   「保持したまま眠ること」で、「眠ること」ではない。**
//!
//! **「スケジューラを入れたときに待たせる形へ変える」という予告は実現した。**
//!
//! # 行の組み立て
//!
...
/// スタックを踏むことのほうが害である。**
///
/// # 数えるのはバイトである（`ADR-0054` の Decision 6）
///
/// **多バイトの字を打つと、入る文字数はそのぶん減る**——**日本語 1 字は
/// 3 バイトなので、この行には 42 字しか入らない。** **文字数で数え直しても
/// 得るものが無い**（上限は覚え書きの大きさの話である）。
