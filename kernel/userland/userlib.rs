...
pub mod heap {
    ...
    /// 取った領域を返してから終わる（H-b-1）。**返す道はこれだけである。**
    ///
    /// # 契約ではなく構造で守る
    ///
    /// **返した領域を後から触れば `#PF` である。** **触らせない保証を
    /// 「呼ぶ側が触らないこと」という契約に置かない**——**契約は、
    /// 守る責任の所在をぼやけさせる。**
    ///
    /// **この関数は戻らない。** **したがって、呼んだ時点から先に、その領域を
    /// 触る命令が 1 つも走らない。** **`-> !` がその保証そのものである**
    /// ——**呼ぶ側が何を持っていようと、続きが無い。**
    ///
    /// **返す道を 1 つに絞ったのはそのためである。** **`unsafe fn release()`
    /// を置けば「終わらずに返す」も書けるが、そこから先は契約になる。**
    /// **いま「終わらずに返したい」利用者は 1 人も居ない**（`zi` は
    /// 終わるときにしか返さない）。**使う者がいない機構は検算が置けない。**
    ///
    /// # 2 人目の利用者が来たら、どこかから保証を持ってくること
    ///
    /// **C の移植で `free` が来ると、「終わらずに返す」が要る。**
    /// **そのときこの `-> !` の保証は使えない。** **契約へ落とさずに済ませる
    /// 形が 1 つある**——**`&'static mut [u8]` を値で受け取る**
    /// （`fn release(region: &'static mut [u8])`）。**一意の参照は Copy では
    /// ないので、渡した側は以後それを使えない。** **所有で守る形である。**
    ///
    /// **ただし [`reserve`] が返した 1 本をそのまま返せる者でないと使えない**
    /// ——**`zi` は割って 2 本にしているので、いまの形では渡し直せない。**
    /// **2 人目が来たときに、割る側と返す側のどちらを直すかを決めること。**
    ///
    /// # 観測はカーネルの側にある
    ///
    /// **成否を返さない**（そもそも戻らない）。**返ったかどうかは
    /// `user-heap:` の行が言う**（`kernel/src/userland.rs`。`brk` が取った数と
    /// 返した数を並べる）。**こちらが「返した」と主張する形は、
    /// 自分で書いて自分で読む形である。**
    pub fn release_and_exit(status: u64) -> ! {
        // SAFETY: この関数は戻らないので、返した領域はこの後で触られない。
        // **契約ではなく、`-> !` が与える構造である**（上の doc）。
        unsafe { release_now() };
        super::exit(status)
    }

    /// 上端を下げる本体（H-b-1）。**外へ出さない。**
    ///
    /// # Safety
    ///
    /// [`reserve`] が返した参照を、この呼び出しの後で使わないこと。
    /// **この契約を負うのは [`release_and_exit`] だけで、
    /// あちらは戻らないことでそれを満たしている。**
    unsafe fn release_now() {
        // SAFETY: 単一の実行文脈である。
