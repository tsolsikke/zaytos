...
/// **W1-c-4 までは単一の直線だったので実害は出なかった。** **W1-c-4 から 2 本の Ring 3 が同時に
/// 走るが、スロット 1 は取らない（下）ので、取り合いはまだ起きない。** **形は先に正しくしてある。**
...
pub(crate) mod script {
    ...
    /// # `a` は全角の上で打つ（`ADR-0054` の Decision 5）
    ///
    /// **`x` の後、カーソルは `u` の上に居る**（`あu` の 2 字目）。
    /// **`h` で全角の上へ戻ってから `a` を打つ**——**そこが要である。**
    /// **`u` の上で打つと、バイトで進めても字で進めても同じ位置になり、
    /// 破壊が捕まらない**（緑の 4 形の「機会が無い」）。
    ///
    /// **`a` は全角の次の境界（3 バイト先）へ動く。** **バイトで進めると
    /// 1 バイト先、つまり字の途中へ落ちる。** **`Z` を入れて保存すると、
    /// 装置の中身が `あZu` ではなく壊れた列になる。**
    ///
    /// **Esc の後に `\x04` を置く**——**Esc だけだと、次の `:` が
    /// エスケープ列の続きとして食われる**（他の台本と同じ形である）。
    ///
    /// # 4 つ目——`$` / `^` / `A` / `o` を多バイトの行で見る（VIM-1）
    ///
    /// **`/data/vimops` は `  あいu` である**（`kernel/build.rs` が書く）。
    /// **先頭の空白 2 つは `^` が飛ばす先で、全角 2 つはバイトと桁を
    /// 食い違わせる**——**`$` と `^` が字の境界に留まることを、同時に
    /// 主張できる形である。**
    ///
    /// **打鍵は `$` `^` `a` Esc `x` `A` `X` Esc `o` `Y` Esc `:wq` である。**
    ///
    /// - **`$` は行末の字（`u`。8 バイト目、6 桁目）へ動く**
    /// - **`^` は最初の非空白（`あ`。2 バイト目、2 桁目）へ動く**
    /// - **`a` を全角の上で打ち、何も入れずに Esc を打つ**——**Esc は
    ///   1 文字ぶん戻るので、`a` の前と同じ 2 バイト目へ帰る。**
    ///   **バイトで戻すと 4 バイト目、つまり `あ` の途中へ落ちる。**
    /// - **続けて `x` を打つ**——**境界に居れば `あ` が丸ごと消えて
    ///   `  いu` になる。** **途中に居ると、字を割って消える**
    ///   ——**実測で、装置の中身の `あ` が不正なバイト列になった**
    ///   （**割った残りがそのまま保存される**）。
    /// - **`A` は行末から挿す**——**`$` と同じ道を通るので、`$` の破壊で
    ///   ここも落ちる**（運用者の指示。2026-09-01）。**`X` を足す。**
    /// - **`o` は `A` の後に Enter を打つのと同じである**——**行が割れて
    ///   `Y` が 2 行目に載る。** **`zi-enter-does-nothing` がここを落とす。**
    ///
    /// **保存後の装置の中身は `  いuX\nY\n` である**（ホスト側が `debugfs`
    /// で読む）。
    ///
    /// **画面は撮らない**（`\x14` を置かない）——**主張は診断の行と
    /// 装置の中身で足りる。** **台本は 22 構成に掛かるので、要らない観測を
    /// 置かない**（運用者の指示）。
