...
/// # 上の一覧とは別のことを言っている
///
/// [`NON_ACQUIRING_ENTRIES`] が言うのは「**この入口は取らない**」である。
/// ここで言うのは「**取る入口が、`Drop` を通らずに出ていく**」ことで、
/// 主張が違う。**一覧のほうへ足すと、あの一覧の主張が偽になる。**
///
/// # 例外の側にこの問題が無い理由
///
/// **畳みも同じ longjmp を通るが、例外経路は BKL を取らない**（上の一覧の
/// `exception`）。取っていないものは解けなくならない。
///
/// # 解いてから出るまでが触るもの
///
/// `ring3::leave_ring3` が触るのは `IN_RING3` と `RECOVERY` だけで、**どちらも
/// 遠征に固有である。** 遠征は AP を起こすより前の単一コア区間に閉じているので、
...
///   **ほとんどは戻らない経路である**（ダンプして停止する）。**戻って写像を使い続ける
///   ことが無いので、古い翻訳が問題にならない。**
///   **例外が1つある**——Ring 3 の遠征の畳み（`ring3.rs`）は制御をカーネルへ戻す。
///   **それでも篩の結果が変わらないのは、遠征が AP 起こしより前の単一コア区間に
///   閉じているからである**（篩の1問目「AP が起きた後に到達しうるか」で落ちる）。
///   **失効条件——Ring 3 が定常状態で走る段（S9）で、この評価をやり直す。**
///
