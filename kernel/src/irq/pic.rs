...
pub fn read_masks() -> (u8, u8) {
    ...
    // 副作用が無いので、同時に読まれても壊れない。**主たる根拠を副作用の無さに
    // 置いてある**ので、AP がこの関数を呼ぶようになっても崩れない。
    // 呼ぶのが起動シーケンスと診断だけであることは、いまの事実として書き添える。
...
///
/// **ハンドラの処理を終えてから呼ぶこと。** EOI を送った時点で PIC は次の
/// 同じ割り込みを上げられるようになる。
///
/// [`EoiAction::SlaveThenMaster`] のときは**スレーブ → マスタの順**に 2 回
/// 送る。この関数が行うのは順序どおりの送信だけで、どの action になるかを
/// 決めるのは [`eoi_action_for`] である。
///
/// # 背景（なぜ両方へ、なぜこの順序か）
///
/// スレーブ側の IRQ（8-15）はマスタの IRQ2 を経由して CPU へ届くため、
/// マスタから見ても 1 件の割り込みが進行中になっている。スレーブにだけ
/// 送るとマスタ側が処理中のまま残り、**以降マスタの全 IRQ が上がらなくなる**
/// （EOI 忘れと同じ症状が、別の IRQ に対して起きる）。
///
/// # Safety
///
...
/// 多重割り込みを許していない現状では両者に差が無い（ハンドラは割り込みゲートで
/// `IF=0` のまま走るので、入れ子にならない）。
