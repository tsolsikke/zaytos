...
//! 生の値を取り上げるためである。呼び出し側は `"pic: IMR after unmasking
...
/// 8259 経由である。この 2 つは EOI の宛先が違う。8259 経由の割り込みは
...
/// # 書き手と読み手
///
/// 書くのは起動時の切り替えだけ（[`route_to_apic`]）。読むのは割り込み
/// ハンドラである（`idt::irq_entry` から EOI とスプリアス判定の経路）。
...
/// ログへ流せる。ベクタオフセットは書いた値であって読み戻した値ではない
...
/// 実測で確認した）。ただし同じベクタ範囲が `sti-check` の要約行
...
/// 閉じるという条件だった。S2 で IO-APIC へ移ればスレーブという概念自体が
/// 消えるので、ここで閉じる。
///
...
/// 初期化である。ここで要るのは無効化で、意図が違う。副作用が一致する
...
/// ある。呼び忘れても警告では気づけないので、roadmap の S2-d-2 の到達条件に
