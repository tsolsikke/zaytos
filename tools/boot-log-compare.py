...
# 比べられないとき——**そう言って降りる**（2026-09-16 に足した）

**この道具は添字で行を対応づける。** **行が増えるか減ると、それ以降が 1 行ずつずれ、
出る数は無意味になる**——**実測で、3 行挿入された段（W2-a）は「伏せても 283 行（50.5%）」と
出た。** **人が「これは使えない」と判断して `diff` で分解した**（`ADR-0061`）。
**黙って無意味な出力を出すより、使えないと言って降りる**（「黙らない」の族）。

**行数の比較だけでは足りない。** **挿入と削除が打ち消し合うと、行数は同じままずれる**
——**実測で、3 行消して別の場所へ 3 行入れた合成は 563 行対 563 行のまま「伏せても 303 行
（53.8%）」だった。** **そこで割合も見る。**

**閾値は 25% にした。** **本物の変更との差が広い**——**実測で、W1-c-4 の段は 4.1%
（559 行中 23 行）、`spawn` の文言を直した回は 0.4%（2 行）だった。** **ずれた側は 50% を
超える。** **境目は測って決めてある。**
...
#: これを越えたら「行がずれている」と見なす割合（上の「比べられないとき」）。
#: **実測で、本物の変更は 0.4% と 4.1%、ずれた側は 50.5% と 53.8% だった。**
MISALIGNED_RATIO_PERCENT = 25.0
...
def main() -> int:
    ...
    if len(old) != len(new):
        print(
            "**行数が違うので、比べられない。** 行が増えたか減っている——"
            "この道具は添字で対応づけるので、以降が 1 行ずつずれて数が無意味になる"
        )
        print("**`diff` で行単位に分解すること**（上の「比べられないとき」）")
        return 3

    differ = [(a, b) for a, b in zip(old, new) if a != b]
    masked = [(a, b) for a, b in differ if HEX.sub("0xADDR", a) != HEX.sub("0xADDR", b)]
    # **行数が同じでも、挿入と削除が打ち消し合っていればずれている**（上の節）。
    ratio = len(masked) * 100.0 / max(len(old), 1)
    if ratio >= MISALIGNED_RATIO_PERCENT:
        print(
            f"**伏せても食い違う行が {len(masked)} 行（{ratio:.1f}%）ある。"
            f"おそらく行がずれているので、比べられない**——"
            "挿入と削除が打ち消し合うと、行数は同じままずれる"
        )
        print("**`diff` で行単位に分解すること**（上の「比べられないとき」）")
        return 3
