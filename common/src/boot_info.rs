...
/// カーネルの静的な初期ページ表が、恒等で張る物理の範囲の終わり（排他。`ADR-0068` の HW-a）。
///
/// **初期ページ表は 2MiB ページ 512 枚で [0, 1GiB) だけを張る**（`kernel/src/main.rs` の
/// `zaytos_boot_pd_shared`）。**自前のページ表へ CR3 を切り替えるまで、カーネルが恒等で触れるのは
/// この範囲だけである。** **ブートローダは受け渡しをこの下へ置き、カーネルは切り替え前に配られた
/// ページ表のフレームがこの下に在ることを確かめる。**
///
/// **初期ページ表の枚数とこの値が一致することを、カーネルがコンパイル時に確かめる**
/// （`kernel/src/main.rs` の `const _: () = assert!(...)`）——**2 か所で別々に書くと、片方だけが動く。**
pub const BOOT_IDENTITY_REACH: u64 = 1 << 30;

/// 受け渡しの領域のうち、メモリマップの写しに使うページ数（`ADR-0068` の HW-a）。
///
/// **ブートローダは ExitBootServices の後にメモリマップを写すので、領域はその前に取る。**
/// **後から広げられないので、見込みに余裕を持たせる**——**16 ページ（64KiB）は、記述子 48 バイトで
/// 約 1,365 本ぶんである**（OVMF の記述子は 48 バイトで、QEMU の既定の構成では 129 本・6,192 バイトだった。起動ログの `descriptors_len`）。
/// **足りなければブートローダは飛ぶ前に止まり、理由を出す。**
pub const HANDOFF_MEMORY_MAP_PAGES: usize = 16;
