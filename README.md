...
## 現在地
M3（画面描画）まで完了。kernel は自前のページテーブル上で動作し、物理
フレームアロケータとカーネルヒープ、そして画面コンソール（`write!` による
文字出力・折り返し・スクロール）が使える。次は M4（割り込み）。
...
[docs/adr/](docs/adr/)（設計判断の記録）を参照。保留した判断は
[docs/deferred-decisions.md](docs/deferred-decisions.md) に解禁条件つきで
一覧化してある。
