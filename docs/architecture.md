...
| 割り込み | 自前 IDT・PIC・PIT を揃えた M4-d で有効化 | ADR-0018 |
...
| M5 の段階分割 | 6 段階。コンテキストスイッチは IRQ スタブの復元経路に載せる | ADR-0019 |
| システムコール ABI | Linux x86-64 のレジスタ規約。入口は `int 0x80` から | ADR-0020 |
| 物理メモリへのアクセス | direct physical map（higher-half 移行後） | ADR-0021 |
...
| 項目 | 既定値 | 意味・影響 |
|---|---|---|
| `disable-redzone` | `true` | レッドゾーン無効。割り込みハンドラが現在のスタックをそのまま使っても、呼び出し元のレッドゾーン領域を破壊する心配がない。**M4 の割り込みハンドラはこの設定に依存している**（下記参照）。 |
...
### 5.1 red zone 無効化への依存（M4 で初めて実際に効く）

`disable-redzone` は M2-0b でターゲットの既定値として確認したが、**この設定が
実際に効くのは M4 の割り込みハンドラが最初**である。red zone（RSP の下
128 バイト）が有効なままだと、リーフ関数がそこにローカル変数を置き、割り込みが
入った瞬間に CPU がスタックフレームを積んで**その領域を破壊する**。

`.cargo/config.toml` の rustflags（`relocation-model=static`,
`code-model=small`）はこの設定を上書きしない。stable では
`--print target-spec-json` が使えないため、**実際のコード生成で確認した**
（2026-07-20 時点、rustc 1.97.1）。

| ターゲット | リーフ関数のスタック使用 | red zone |
|---|---|---|
| `x86_64-unknown-none`（kernel） | `subq $64, %rsp` して正のオフセット | **無効** |
| `x86_64-unknown-linux-gnu`（ホスト） | RSP を動かさず `-8(%rsp)` を直接使用 | 有効 |

rustflags やターゲットを変更した際は、この確認をやり直すこと。
...
### 6.3 kernel のスタックについて（M4-a で対応済み）

M3 までの kernel は、bootloader が実行していた UEFI 由来のスタックをそのまま
使い続けていた。M4-a で自前のスタックへ切り替えた（ADR-0018）。

- 通常実行用 64KiB と、ダブルフォルト（IST1）用 16KiB を `.bss` の静的配列
  として確保する。フレームアロケータより前に切り替えられること、kernel
  イメージの一部として既に予約済み・マップ済みであることが理由。確保の
  失敗経路が存在しない。
- 切り替えは `_start` の最初期に行う。旧スタックの上に状態を積むほど、
  切り替え後に「参照してはいけない領域」が増えるため。引き継ぐ値は
  スタックではなく静的領域（`BOOT_HANDOFF`）経由で渡す。
- 切り替え後、RSP とローカル変数の実アドレスがともに自前スタックの範囲内に
  あること、旧スタックを参照していないこと、実際に読み書きできることを
  起動時に検証してログへ出す（CR3 切り替えと同じ作法）。
- スタックオーバーフローの検出は、当面**犠牲領域 + 毒値カナリア**で行う
  （各スタックの直下に 4KiB、`0xC5` で充填）。ガードページ化は
  `deferred-decisions.md` を参照。
- **配置順は `#[repr(C)]` の単一構造体で固定する。** 静的変数の配置をリンカ
  任せにすると、たまたま重要なものがスタックの直下に来る。実際、最初の実装では
  カーネルスタックの直下がヒープの `ALLOCATOR` と `TSS` になっており、
  オーバーフローが「検出のための仕組みそのもの」を最初に壊す並びだった
  （TSS が壊れると IST が失われ、ダブルフォルトがトリプルフォルトになる）。
- kernel イメージの予約は 43 ページ → 67 ページ（+96 KiB）になった。内訳は
  スタックブロック 88 KiB（犠牲 4KiB + 通常 64KiB + 犠牲 4KiB + IST 16KiB）、
  TSS・GDT・引き継ぎ領域で 176 バイト、コードとアラインメントで約 7.9 KiB、
  最後にページ境界へ切り上げ。
...
- CR3 は M2-d (d-2) で自前のテーブルへ切り替え済み。UEFI 由来の旧
  ページテーブルは回収していない。
- 稼働中のテーブルは起動時に walk して読み戻し、計画と突き合わせる
  （M5-a-1、`paging::active`）。エントリのビット操作（2MiB と 4KiB で
  位置が違う PAT の移送を含む）は `paging::entry` に分離し、ホストテストで
  固定している。
...
### 6.7 描画系の確定構成（M3 完了時点）

M3 で実装した描画とコンソールの現在の姿。経緯は ADR-0013 / 0016 / 0017。

**graphics 層**（`kernel/src/graphics/`）
- `layout`: bootloader から渡された形状を検証し、通ったものだけを
  `FramebufferLayout` として返す。検証項目はピクセル形式、物理アドレスが
  0 でなく 4 バイト境界にあること、幅と高さが 0 でないこと、
  `stride >= width`、`height * stride * 4 <= size_bytes`、桁溢れ。
  この型が存在すること自体が検証済みの証明になる。
- `color`: `Color` から `Rgb`/`Bgr` に応じた 32bit 値への変換。生の `u32` を
  コード中に書かない（形式を取り違えると赤と青が入れ替わる）。
- `framebuffer`: `write_volatile` による書き込み。画面外は無視し、はみ出した
  矩形は切り詰める。描画経路では panic しない。
- `font`: GNU Unifont のグリフ検索。半角 8x16 / 全角 16x16 を区別し、鍵は
  `char`。未収録は U+FFFD へフォールバックする。
- `text`: 文字列の描画。改行・折り返しは扱わない。

扱うのは `Rgb`/`Bgr` の 32bpp のみ。`Bitmask` と `BltOnly` は起動時に拒否する。

**console 層**（`kernel/src/console/`）
- `grid`: カーソルと桁送り。全角は行末に 1 セルしか残っていなければ次行へ
  送る（半端に描かない）。構築時に桁数 >= 最も広いグリフ幅を検証する。
  制御文字は `\n` / `\r` / `\t` のみ。`\r` は行頭復帰に加えて行を消去する
  （厳密な端末挙動からの意図的な逸脱）。
- `dirty`: 未転送範囲を外接矩形 1 個で追跡し、画面内へ切り詰める。
- `backbuffer`: 通常 RAM 上の、フレームバッファと同一形状・同一 stride の面。
  転送は行ごとの `copy_nonoverlapping` で行い、直後に `compiler_fence` を
  置く。一括操作（全面クリア・スクロール・行消去）はスライスの
  `fill` / `copy_within` を使う。
- `screen`: 上記を束ねた `Console`。`core::fmt::Write` を実装し、
  `write_fmt` を override して `write!` 1 回につき 1 回だけ転送する。

**描画とフラッシュの流れ**
- 描画はすべてバックバッファ（キャッシュ有効）に対して行う。
- フレームバッファ（PCD、キャッシュ無効）へは変更範囲だけを転送する。
- スクロールはバックバッファ内のメモリコピーで行い、画面全体が変わるため
  その回だけ全面転送になる。
- 実測（1280x800、起動シーケンス）では、コンソール構築からタイマ開始までに
  11 回のフラッシュで 4,402,688 バイト。毎回全面転送なら 45,056,000 バイト
  なので 9%。スクロールが起きる負荷（62 行の連続出力）でも 37% だった。

**コンソールの位置づけ**
- グローバルに置かず、ロックも持たない。割り込みハンドラから出力する要求は
  M4 で生じるため、そのロック設計は割り込み安全性のモデルと一体で決める。
- シリアルログはコンソールから完全に独立している。`common::log::Logger` に
  複数の出力先は持たせない。両方へ出す場合は呼び出し側のヘルパで行い、
  **必ずシリアルを先に**書く。
- コンソールはフレームアロケータと新ページテーブルを必要とするため、起動
  シーケンスの途中でしか作れない。それより前のログはシリアルにしか出ない
  （実機の Linux で printk のログバッファとコンソールドライバ登録の関係が
  同じ構造）。画面の先頭に、省略された行数を出して誤解を防ぐ。
- パニック時に画面へは出さない（ADR-0013 Addendum）。

---

### 6.8 割り込み系の確定構成（M4 完了時点）

M4 で実装した割り込み基盤の現在の姿。経緯は ADR-0018。

**GDT / TSS / スタック**（`kernel/src/gdt/`, `kernel/src/stack.rs`）
- null / カーネルコード / カーネルデータ / TSS（16 バイトなので 2 枠）の
  5 スロット。ユーザーモード用は未追加（SYSCALL の並び順制約は
  `deferred-decisions.md`）。
- スタックは `.bss` の静的配列。通常 64KiB、ダブルフォルト用 IST1 が 16KiB。
  各スタックの直下に 4KiB の犠牲領域を置き毒値 `0xC5` で埋める。
  配置順は `#[repr(C)]` の単一構造体で固定する。
- 起動時に `sgdt` / `str` / セグメントレジスタを**読み戻して**照合する。

**IDT とスタブ表**（`kernel/src/idt/`）
- 全 256 ベクタを埋める。割り込みゲート（0xE）・DPL 0。
- **スタブ表は 2 系統ある。** 例外用 256 本（戻らない）と、IRQ スタイル
  17 本（GPR を復元して `iretq` で戻る。0x20-0x2F の PIC IRQ と、
  テスト専用の 0x30）。例外を IRQ 化してはならない（#DE から戻れば同じ
  除算命令を再実行して無限ループになる）。**配置検証も 2 系統**で、
  片方の検証がもう片方を保証しない。
- `call` 直前の RSP が 16 バイト境界にあることを、スタブが読んだ**実測値**で
  検証する。導出は ADR-0018 Addendum 2。

**クリティカルセクション**（`common/src/critical.rs`）
- `InterruptGuard` は RAII で `cli`/`sti` を対にし、**保存時に IF=1 だった
  場合のみ**復元する（入れ子で正しく動く）。
- `Locked<T>` は取得中だけ割り込みを禁止する。ガードのフィールド順で
  「割り込みの復元が必ず最後」を保証し、drop 順をホストテストで固定。
  二重取得は `AtomicBool` で検出し、panic ではなくシリアルへ直接出して停止
  （ヒープのロックで起きた場合に panic 経路が確保を試みると悪化するため）。

**PIC**（`kernel/src/pic.rs`）
- 8259A を 0x20-0x2F へ再マップ。**ICW2 は書き込み専用で読み戻せない**ため、
  オフセットの正しさは最初のタイマ割り込みがベクタ 0x20 で届いたことで
  事後的に証明する（`alt-offset-test` で 0x30-0x3F への追随も確認済み）。
- IMR は読み戻して照合する。解禁は IRQ ごとに 1 箇所へ集約。
- スプリアス（IRQ7 / IRQ15）は ISR を読んで判定し、偽には EOI を送らない。
  **IRQ15 の偽だけはマスタにのみ送る**（マスタはカスケードを本物として
  受けているため）。

**PIT**（`kernel/src/pit.rs`）
- チャネル 0、モード 3、100Hz（分周値 11932、実周波数 99.998Hz）。
  **分周値が整数のため周期は要求値と一致しない。** ティック数 × 10ms を
  時刻として使ってはいけない。

**i8042 / キーボード**（`kernel/src/keyboard/`）
- コンフィグバイトを読んで翻訳（セット 1）と割り込みが有効かを確認し、
  必要なら立てて**読み直す**。自己テスト（0xAA / 0xAB）は行わない。
- IRQ1 解禁の前に出力バッファをドレインする。
- ハンドラは**バッファが満杯でもデータポートを必ず読む**。溢れたときは
  新しい方を捨てて数える。データを伴わない IRQ1（コントローラ応答の余波）は
  `stray` として別に数える。
- 変換（セット 1 → 文字）は純粋ロジックでホストテスト対象。

**メインループ**（`kernel/src/interrupts.rs`）
- `sti; hlt` を隣接させたヘルパで**無条件に眠る**。仕事の有無で眠りを
  決めないので `cli` → 確認 → `sti; hlt` の形は要らない。**起こすものが
  止まると永久ハングになるが、カーネル自身は検出できない**（検出コードが
  動かない）。外側から xtask がハートビート本数で判定する。
- ハンドラは何も出力しない（ADR-0018 §5）。観測はカウンタ越しに行い、
  ハートビート 1 行に集約する。ハートビートには**会計**（`keys + stray` が
  ハンドラ呼び出し回数と IDT のベクタ別カウンタに一致するか）と、
  **止まった理由の切り分け材料**（i8042 の OBF、PIC の ISR）を含める。
- `sti` の直前に ADR-0018 §2 の 7 項目を実行時検証し、1 つでも欠ければ
  `sti` せず停止する。判定は `Verified` / `Failed` / `Unverifiable` の
  3 状態で、確かめる手段が無いものを真に丸めない。
