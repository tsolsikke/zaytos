...
- 高位化が要る（B-2b-1/2で高位化。恒等生存中は高位VAと低位VAが同じ物理を指すので振る舞い不変）:
  (2) boot_infoの低位デレフ [464行] `&*handoff.boot_info`。(1-cascade) 必須マッピング検証のboot_info_start [754行]。boot_infoを高位化すると754行が高位になりidentity()クロージャが落ちるので、物理はhandoff.boot_infoの低位生値から取る（**恒等前提の修正が別の恒等前提を破る連鎖**。(1)止まりの採番が危険だった実例）。(3) ヒープinit [1048行] `ALLOCATOR.init(heap_start)`。
- 除去より前に走るので不変（B-2b-3で順序を明示的に守る）: (5-8) 恒等窓を握る4検証サイト [user-page/ring3/syscall-roundtrip/syscall-pointer]。(11) kernel_startの低位read [858/896行、M2-d切替後検査]。(4) run_paging_testのheap_virt [paging-test feature下のみ、1087行で除去前に走る]。
- 除去を破らない（対象外）: (9) 初期direct_map登録 [476行、A-2で高位窓へ差し替え済み]。(10) old_rspの範囲検査 [1780行、値の包含比較でデレフなし]。パニック/例外経路（恒等非依存。後述）。

**コードとこの一覧の同期。** かつて「コード内採番を始めて途中でやめた」（`main.rs:480`の`(1)`止まり）ことが列挙の未完を招いた。同じ失敗を繰り返さないため、採番はコードから外しこの一覧を正とする。高位化が要る箇所（上記の第1群）はコードに固有マーカー `恒等前提の箇所` を置いてある。**`grep '恒等前提の箇所' kernel/src/main.rs` の結果と、この一覧の「高位化が要る」群および除去手順ステップ4の必須領域リストは一致していなければならない**（新しく高位化する低位ポインタを増やしたら、コードのマーカーとこの一覧と必須領域リストの3つを同時に足す）。
...
4. 独立walkerで検証。必須領域リスト=常に必要（RIP/RSP/カーネルイメージ/direct map窓/フレームバッファの高位VA）+ 列挙由来（(2)boot_infoの高位VA、(3)ヒープの高位VA）が恒等なしで解決し、かつ`PML4[0]`が空であること。**このリストは恒等前提の列挙と同期している必要がある**（新しく高位化したポインタを増やしたらリストにも足す。この安全網の前提）。読み戻しは**walkで**行う（「低位VAが未マップ」をデレフで試すとフォルトするので、テーブルを辿ってNotPresentを読む）。
