...
- 高位化が要る（B-2b-2で高位化。恒等生存中は高位VAと低位VAが同じ物理を指すので振る舞い不変）:
  (3) ヒープinit [1048行] `ALLOCATOR.init(heap_start)`。heap_startは物理値で、恒等の間だけ低位VAとして通る。**除去後もタイマループ・タスク・コンソールの全アロケーションで使われ続けるので、これが唯一のgenuineな高位化対象。** `phys_to_virt(heap_start)`の高位VAでinitする。
...
**コードとこの一覧の同期。** かつて「コード内採番を始めて途中でやめた」（`main.rs:480`の`(1)`止まり）ことが列挙の未完を招いた。同じ失敗を繰り返さないため、採番はコードから外しこの一覧を正とする。恒等前提の箇所はコードに固有マーカー `恒等前提の箇所` を置いてある（現状: boot_info[464]・必須マッピング検証[745]・image_phys[801]・ヒープ[1048]）。`grep '恒等前提の箇所' kernel/src/main.rs` で引ける箇所は、すべてこの一覧のいずれかのカテゴリに現れていなければならない（マーカーと一覧の同期）。そのうち「高位化が要る」群（現状はヒープのみ）と除去手順ステップ4の必須領域リストは**一致していること**（新しく高位化する低位ポインタを増やしたら、コードのマーカー・この一覧・必須領域リストの3つを同時に足す）。
