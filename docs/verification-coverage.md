...
**コードとこの一覧の同期。** かつて「コード内採番を始めて途中でやめた」（`main.rs:480`の`(1)`止まり）ことが列挙の未完を招いた。同じ失敗を繰り返さないため、採番はコードから外しこの一覧を正とする。恒等前提の箇所はコードに固有マーカー `恒等前提の箇所` を置いてある。現状 `grep '恒等前提の箇所' kernel/src/main.rs` = 3件で、各々「なぜ未解決側に残るか」がコメントと本一覧に書いてある: boot_info[468]（除去前に消費・468<477で高位化不可）、必須マッピング検証[745]（BootInfo_startを低位=物理として見る恒等前提・除去より前に走る）、kernel_start低位read[862]（順序依存・862<991で高位化不可・記録で守る）。解決した箇所はマーカーを「恒等前提だった箇所（…で解消）」に変えてgrepから外す（ヒープ[B-2b-2]・image_physのRSP/RIP変換[B-2a-2]がその例）。同期の不変条件を精密に書く。**`grep '恒等前提の箇所'` の結果は、この一覧のうち「まだ恒等前提が残っている（未解決の）箇所」の部分集合である。** 解決した箇所（高位化した等）はコードのマーカーを「恒等前提だった箇所（…で解消）」へ変えてgrepから落とす。これで双方向に食い違いを検出できる: 新しい恒等前提を足したのに一覧へ書き忘れれば、grepに出て一覧に無い（検出）。解決したのに一覧を直し忘れれば、grepから消えて一覧では未解決のまま残る（検出）。「高位化した」群と除去手順ステップ4の必須領域リストも一致していること（新しく高位化する低位ポインタを増やしたら、コードのマーカー・この一覧・必須領域リストを同時に更新する）。
