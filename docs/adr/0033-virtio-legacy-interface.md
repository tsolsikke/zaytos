...
direct mapウィンドウが覆うのは物理RAM（256MiB）とAPICのMMIOだけで、BAR4はその外にある。modernで話すには、この領域を新しくマップする必要がある。legacyで話すなら、ポートI/Oの土台（S13-aの`0xCF8`/`0xCFC`で実績がある`common::port`）だけで足り、マッピングは一切要らない。

## Decision

**virtio装置とはlegacy interface（BAR0のI/Oポート）で話す。**

- 初期化のハンドシェイク・feature交渉・queueの設定・notifyのすべてをBAR0経由で行う
- featureは何も受けずに交渉する（装置側のfeature bitsは読んで判定行に出す。観測はする）
- リングの物理アドレスはQueueAddressレジスタへPFN（物理アドレス右シフト12）で渡す

決めないこと（ADR-0025と同じ形。各段階の着手時に決める）:
...
- **ブロック層の境界（イメージ全体のロード / ブロックキャッシュ）**——S13-cの着手時
- **書き込みの経路（`VIRTIO_BLK_T_OUT`、flushの扱い）**——S13-e
- **複数キュー・複数リクエスト**——要る場面が出るまで

**いずれも本ADRのDecisionからは導かれない**——legacyで話すことは、割り込みの届き方・イメージの供給のされ方・要求の種類・キューの本数を何も制約しない。導かれるなら、それは決めていることである。

## Alternatives Considered

**modern interface（BAR4のMMIO）——却下。**

- BAR4（`0xc000000000`）はdirect mapの外で、新しくマップする必要がある。`deferred-decisions.md`の「4KiBを1枚張る経路が2つあることの統合」が示すとおり、同じ操作の実装は既に2経路あり、**3つ目を作らない**
...
- MMIOのマッピングが要らない。ポートI/Oだけで完結し、S13-bの範囲が縮む
...
再訪条件: **virtio 1.0でしか使えない機能が要るとき**（packed queue、上位のfeature bit、64bitのqueueアドレス指定など）。そのときこのADRをSupersedeし、modernへの移行とマッピングの経路の判断を一緒に行う。

## Addendum（2026-08-28。P-c。1回の要求で2MiBを書ける根拠）

**P-cで、シェルの文脈からイメージを書き戻す。** **いまの起動シーケンスのフラッシュは4KiBずつ512回だが、1回の要求で2MiB全部を書けることを実測した**（`--fs-extract`が通り、`disk_matches_dump`も`e2fsck`も無傷と判定した）。**BKLを解いて眠る踊りが512回ではなく1回で済むので、この差は大きい。**
