...
2. **「設定する側が無い」は言い過ぎだった。** `route`はS2-d-1cから`redirection_flags_for_irq`をORしており、**overrideの旗を流す機構は最初から在った。** 無かったのは「overrideの無いIRQへバスの規定を与える既定」と「宣言と読み戻しの一致の判定」である。持ち越しの行はgrepの語（`level`/`polarity`）で機構を探して見落としていた——**キーワードgrepは補助であって主ではない**（`CLAUDE.md`の監査の規律と同じ）

**QEMUは極性とトリガを厳密に模っていない**（生のエッジ・ハイで書いても届く。実測）。したがって「届いた数」は配送の実証にはなるが、**鳴り方の誤りを捕まえるのは読み戻しだけである**——破壊`virtio-intx-edge-test`はそこで落ちる。実機では鳴り方の誤りは配送そのものを壊すはずで、そのときは届いた数の判定も効く。
