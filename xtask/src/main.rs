...
fn cmd_lapic_timer_test(kind: &str) -> Result<()> {
    ...
    let mut child = launch::spawn(&launch::Spec::new(
        ...
        &outputs,
        "lapic-timer-test",
        EXCEPTION_TEST_TIMEOUT + LAPIC_TIMER_MEASURE_WINDOW,
        launch::Deadline::Failure,
    ))?;
...
fn cmd_kernel_entry_concurrency() -> Result<()> {
    ...
    let mut child = launch::spawn(&launch::Spec::new(
        &qemu_args,
        &outputs,
        "kernel-entry-concurrency",
        KERNEL_ENTRY_CONCURRENCY_TIMEOUT,
        launch::Deadline::Failure,
    ))?;
...
/// 1.6倍である。**
///
/// # 120分から170分へ上げた（2026-09-18。`ADR-0063`。運用者の承認）
///
/// **規則は変えていない。いちばん遅い実測を今の値にしただけである。**
/// **いちばん遅い実測は壁時計106.1分（311項目。2026-09-17）で、1.6倍は169.8分である。**
///
